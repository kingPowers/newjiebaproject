...
class Model {
     ...
     * @access public
     +----------------------------------------------------------
     */
    public $page = array('no'=>1,'num'=>15,'total'=>0,'records'=>0);
    
    public $simple_page = array('no'=>1,'num'=>5,'total'=>0,'records'=>0);
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        //转换小分类到大分类、
        /*$data['category_admin'] = $data['cateid'];
        $find_where['id'] = $data['cateid'];
        $loanInfo = M('loan_category')->field('category_id')->where($find_where)->find();
        $data['cateid'] = $loanInfo['category_id'];*/
    ...
    public function select($options=array()) {
        ...
        if(isset($options['limit'])){
        	$limit = $options['limit'];
        	unset($options['limit']);
        	$records = $this->db->select($options);
        	if(is_array($records)){
        		$this->page['records'] = count($records);
        		$this->page['total'] = ceil($this->page['records']/$this->page['num']);
        	}
        	$options['limit'] = $limit;
        }
        */
	...
	*/
	public function inTrans(){
		return ($this->db->transTimes > 0) ? true : false;
	}
