...
class Baofu {
	...
	private $backTransRequest;//认证类接口
	private $private_key_path;//私钥文件路径
	private $public_key_path;//公钥文件路径
	private $private_key_password;//私钥文件密码
	/*
	 * 代付类配置
	 * 
	 * */
	private $payVersion = "4.0.0";//代付类版本号
	private $pay_terminal_id;//终端号
	private $pay_private_key_path;//私钥文件路径
	private $pay_public_key_path;//公钥文件路径
	private $pay_private_key_password;//私钥文件密码
	private $bfpayBF0040001;//代付交易URL
	private $bfpayBF0040002;//代付交易状态查询URL
	/*
	 * 代扣类配置
	 * 
	 * */
	private $collectVersion = "4.0.0.0";//代扣类版本号
	private $collect_terminal_id;//终端号
	private $collect_private_key_path;//私钥文件路径
	private $collect_public_key_path;//公钥文件路径
	private $collect_private_key_password;//私钥文件密码
	private $bfcollectBF0040001;//代扣交易URL
	private $collectSearchUrl;//代扣交易查询URL
	/*
         * $memberid:用户会员ID
         */
	function __construct($memberId){
		$this->memberid = $memberId;
                if(empty($this->memberid)){
                    throw new Exception("参数memberid为空");
                }
                if(false==($member = M("member")->where(["id"=>$this->memberid])->find())){
                    throw new Exception("memberid用户不存在");
                }
                $this->businessCompanyId = $member["business_company_id"];
                $this->setParams();
	}
	/*
         * 参数配置
        ...
        private function setParams(){
            if($this->debug){//调试环境
                $this->mchnt_cd = "100000276";//商户号
                $this->terminal_id = "100000990";//认证类终端号
                $this->pay_terminal_id = "100000859";//代付类终端号
                $this->collect_terminal_id = "100000933";//代扣类终端号
                
                $this->backTransRequest = "https://tgw.baofoo.com/cutpayment/api/backTransRequest";//认证类接口
                $this->bfpayBF0040001 = "http://paytest.baofoo.com/baofoo-fopay/pay/BF0040001.do";//代付交易URL
                $this->bfcollectBF0040001 = "https://tgw.baofoo.com/cutpayment/api/backTransRequest";//代扣交易URL
                $this->bfpayBF0040002 = "http://paytest.baofoo.com/baofoo-fopay/pay/BF0040002.do";//代付交易状态查询URL
                $this->collectSearchUrl = "https://vgw.baofoo.com/cutpayment/api/backTransRequest";//代扣交易查询URL
            }else{//正式环境
                $businessSet = M("business_baofu_set")->where(["business_company_id"=>$this->businessCompanyId])->find();
                $this->mchnt_cd = "";//商户号
                $this->terminal_id = "";//认证类终端号
                $this->collect_terminal_id = "";//代扣类终端号
                
                 $this->backTransRequest = "https://public.baofoo.com/cutpayment/api/backTransRequest";//认证类接口
                 $this->bfpayBF0040001 = "https://public.baofoo.com/baofoo-fopay/pay/BF0040001.do";//代付交易URL
                 $this->bfcollectBF0040001 = "https://public.baofoo.com/cutpayment/api/backTransRequest";//代扣交易URL
                 $this->bfpayBF0040002 = "https://public.baofoo.com/baofoo-fopay/pay/BF0040002.do";//代付交易状态查询URL
                 $this->collectSearchUrl = "https://public.baofoo.com/cutpayment/api/backTransRequest";//代扣交易查询URL
            }
	...
	private function setSecretKeyPath(){
                if($this->debug){//调试环境
                    $this->private_key_path = dirname(FRAME_PATH).'/cer/bfkey_100000276@@100000990.pfx';//认证类公钥
                    $this->public_key_path = dirname(FRAME_PATH).'/cer/bfkey_100000276@@100000990.cer';//认证类私钥
                    $this->private_key_password = "123456";//认证类私钥文件密码
                    
                    $this->pay_public_key_path = dirname(FRAME_PATH).'/cer/pay_baofoo_pub.cer';//代付类公钥
                    $this->pay_private_key_path = dirname(FRAME_PATH).'/cer/pay_m_pri.pfx';//代付类私钥
                    $this->pay_private_key_password = "123456";//代付私钥文件密码
                    
                    $this->collect_private_key_path = dirname(FRAME_PATH).'/cer/bfkey_100000749@@100000933.pfx';//代扣类私钥
                    $this->collect_public_key_path = dirname(FRAME_PATH).'/cer/bfkey_100000749@@100000933.cer';//代扣类公钥
                    $this->collect_private_key_password = "100000749_272769";//代扣类私钥文件密码
                }else{//正式环境
         ...
         * 创建订单
         *    创建宝付订单
         *     @param array orderParams 订单信息
         *                  tender_number:贷款订单编号，必须
         *                  trans_type:交易类型，必须
         *                  trans_money：交易金额，必须
         *                  
         *                                   
         *                  
         *     @return array orderInfo 返回订单信息
         * 
         *     订单创建成功返回订单数组，否则返回false
         * 
         */
        public function createOrder($orderParams = []){
            if(empty($orderParams) || empty($orderParams["tender_number"])){
                $this->error = "订单号不存在";
                return false;
            }
            if(empty($orderParams["trans_type"])){
                $this->error = "交易类型不存在";
                return false;
            }
            $arrContent = ["1"=>"打款","2"=>"还款"];
            if(!in_array($orderParams["trans_type"], array_keys($arrContent))){
                $this->error = "交易类型不正确";
                return false;
            }
            $orderInfo = M("baofu_order")->where(["tender_number"=>$orderParams["tender_number"],"trans_type"=>$orderParams["trans_type"]])->find();
            if(false!=$orderInfo && ($orderInfo["trans_result"]=="未交易" || $orderInfo["trans_result"]=="交易成功")){
                return $orderInfo;
            }elseif(false!=$orderInfo && $orderInfo["trans_result"]=="交易失败"){
                $orderParams["trans_no"] = $orderParams["trans_no"]?$orderParams["trans_no"]."_".substr($orderInfo["trans_no"], -1):
                                           substr($orderInfo["trans_no"],strlen($orderInfo["trans_no"])-1).(intval(substr($orderInfo["trans_no"], -1))+1);
            }
            
            if(round($orderParams["trans_money"],2)<=0){$this->error = "交易金额不正确";return  false;}
            $orderParams["trans_no"]  = $orderParams["tender_number"]."_".$orderParams["trans_type"]."_1";
            $orderParams["trans_serial_no"]  = $orderParams["trans_serial_no"]?$orderParams["trans_serial_no"]:time().rand(111,9999).rand(11,99);//流水号
            $orderParams["trans_content"]  = $orderParams["trans_content"]?$orderParams["trans_content"]:$arrContent[$orderParams["trans_type"]];
            $orderParams["trade_date"]  = $orderParams["trade_date"]?$orderParams["trade_date"]:date("YmdHis");
            $addOrder = [
                "tender_number"=>$orderParams["tender_number"],
                "trans_money"=>$orderParams["trans_money"],
                "trans_no"=>$orderParams["trans_no"],
                "trans_serial_no"=>$orderParams["trans_serial_no"],
                "trans_type"=>$orderParams["trans_type"],
                "trans_content"=>$orderParams["trans_content"],
                "trade_date"=>$orderParams["trade_date"],
            ];
            if(false==($addId = M("baofu_order")->add($addOrder))){$this->error = "订单未创建成功;sqlerror:".M()->getDbError().";sql:".M()->getLastSql();return false;}
            return M("baofu_order")->where(["id"=>$addId])->find();
        }
	...
	public function preBindCard($cardData){
		...
		if(false==($responseArr = $this->postOfResponse($this->backTransRequest,$postArr,"认证-预绑卡获取验证码")))return false;
		$saveArr = array(
	...
	* */
	...
	public function bfPay($trans_no,$trans_money,$remark = ""){
		if(false==($bindInfo = $this->bindBankCardStatus()))return false;
                ...
                if(false!==($searchResult = $this->bfPaySearch($trans_no))){
                    if(0===(int)$searchResult["state"]){//付款中订单，不可再次代付
                        ...
                        M("baofu_order")->where(["trans_no"=>$trans_no])->save(["trans_result"=>"交易成功","trans_return"=>json_encode($searchResult)]);
		...
		$postArr = array();
		$postArr['member_id'] = "100000178";//商户号，必填
		...
		$result = $this->payPostOfResponse($this->bfpayBF0040001,$postArr,"代付-代付交易");
                if(false==$result)return false;
                M("baofu_order")->where(["trans_no"=>$trans_no])->save(["trans_result"=>"交易成功","trans_return"=>json_encode($result)]);
                return $result;       
	}
        /*
         * 宝付代付订单查询
	 ...
	 *  @param $trans_no:订单号
	 *  @param $trans_money:交易金额，单位：元
         *  @param $trans_serial_no:商户流水号
         *  @param $trade_date:交易日期
	 *  @return boolean|array
	 * */
	public function bfCollect($trans_no,$trans_money,$trans_serial_no ="",$trade_date = "",$remark = ""){
		if(false==($bindInfo = $this->bindBankCardStatus()))return false;
                //交易成功的订单号直接返回array
                if(false!==($searchResult = $this->collectSearch($trans_no,$trans_serial_no,$trade_date))){
                    if("I"==$searchResult["order stat"]){//代扣中订单，不可再次代付
                        $this->error = "订单处理中";
                        return false;
                    }elseif("S"==$searchResult["state"]){//代扣成功订单，不可再次代付
                        M("baofu_order")->where(["trans_no"=>$trans_no])->save(["trans_result"=>"交易成功","trans_return"=>json_encode($searchResult)]);
                        return $searchResult;
                    }
                }
                $trans_serial_no = $trans_serial_no?$trans_serial_no:rand(111,999).time();
                $trade_date = $trade_date?$trade_date:date("YmdHis");
		$encData = array();
		...
		$encData['member_id'] = "100000749";//商户号，必填
		...
		$encData["trans_serial_no"] = $trans_serial_no;//商户流水号，必填
		$encData["trade_date"] = $trade_date; //订单日期，必填
		...
		$postArr['member_id'] = "100000749";//商户号，必填
		$postArr['data_type'] = $this->data_type;//加密数据类型，必填
		$postArr['data_content'] = $this->rsaSign($encData,$this->collect_private_key_path,$this->collect_private_key_password);//加密数据，必填
		$result =  $this->collectPostOfResponse($this->bfcollectBF0040001,$postArr,"【{$remark}】代扣-代扣交易");
                if(false==$result)return false;
                M("baofu_order")->where(["trans_no"=>$trans_no])->save(["trans_result"=>"交易成功","trans_return"=>json_encode($result)]);
                return $result;
	}
         ...
         *      
         */
        public function collectSearch($trans_no,$trans_serial_no,$trade_date){
            $encData = array();
            $encData['txn_sub_type'] = "31";//交易子类，必填
            $encData['biz_type'] = "0000";//接入类型，必填
            $encData["terminal_id"] = $this->collect_terminal_id;//终端号，必填
            $encData['member_id'] = "100000749";//商户号，必填
            $encData["trans_serial_no"] = $trans_serial_no;//商户流水号，必填
            $encData['orig_trans_id'] = $trans_no;//订单号，必填
            $encData['orig_trade_date'] = $trade_date;//订单号，必填
            dump($encData);
            $postArr = array();
            $postArr['version'] = $this->collectVersion;//版本号，必填
            $postArr['terminal_id'] = $this->collect_terminal_id;//终端号，必填
            ...
            $postArr['data_type'] = $this->data_type;//加密数据类型，必填
            $postArr['data_content'] = $this->rsaSign($encData,$this->collect_private_key_path,$this->collect_private_key_password);//加密数据，必填
            dump($postArr);dump($this->collectSearchUrl);
            return $this->collectPostOfResponse($this->collectSearchUrl,$postArr,"【{$remark}】代扣-代扣交易查询");
        }
	...
	public function getError(){
		$err = array(
				...
				"报文交易要素格式错误密文域中参数acc_no校验失败"=>"银行卡号不正确11",
	 ...
	 * @retutn boolean
	 * */
	private function checkBankCard($cardNo){return true;
