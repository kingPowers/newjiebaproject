...
<body>
<style type="text/css">
.title{display: inline-block;width: 150px;height: 30px;text-align: center;line-height: 30px;margin: 20px 0 0 0px;position: relative;top :9px;}
</style>
<script type="text/javascript" src="_STATIC_/2015/js/wdate/WdatePicker.js"></script>
<a href="<?php echo U('BusinessCompany/depositeDetail',array('company_id'=>$_REQUEST['company_id']));?>" class='title btn' style="margin-left: 10px;">新增</a><span  class="btn title btn-primary">支出</span>
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tbody>
        <?php if(is_array($list)): foreach($list as $k=>$vo): ?><tr>
            <td><?php echo ($vo['costout_number']); ?></td>
            <td><?php echo ($vo['cost_money']); ?></td>
            <td><?php echo ($vo['total_balance']); ?></td>
            <td><?php echo ($vo['timeadd']); ?></td>
            <td><a href="/#25/36?tender_id=<?php echo ($vo['tender_id']); ?>&memberid=<?php echo ($vo['memberid']); ?>" target="_blank">查看详情</a></td> 
