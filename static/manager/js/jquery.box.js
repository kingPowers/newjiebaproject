$.getScript(STATIC+"/manager/js/wbox.js");
...
var jdbox = {
	...
	alert : function(type,content,back,is_iframe){
		var title ='',msg ='',bwp = '',titleClass='default',back=back||'';
		if(type==0){
			title = '错误';
			msg = '<div class="error-wp"><span style="font-size:18px;">'+content+'</span></div>';
			bwp = '<a href="javascript:;" onclick="jdbox.close('+is_iframe+');'+back+'" class="bt-error">确定</a>';
			titleClass = 'error';
		}else if(type==1){
			title = '成功';
			msg = '<div class="success-wp"></span><span style="font-size:18px;">'+content+'</span></div>';
			bwp = '<a href="javascript:;" onclick="jdbox.close();'+back+'" class="bt-success">确定</a>';
		}else if(type==2){
			title = '加载中...';
			msg = '<div class="wait-wp"><p><span class="icon"></span></p><p style="font-size:18px;margin-bottom:40px;">'+(content?content:'')+'</p></div>';
		}
		var html  = '<div class="alert-wp">'+msg+'</div><div class="alert-button-wp">'+bwp+'</div>';
		$("body").append('<input class="boxactiver" style="display:none;"><input class="wBox_close boxcloser" style="display:none;">');
		$(".boxactiver:last").wBox({'title':title,'html':html,'titleClass':titleClass,'is_iframe':is_iframe}).click();
	},
